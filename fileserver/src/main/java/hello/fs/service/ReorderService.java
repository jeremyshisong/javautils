...
public class ReorderService {
    ...
    public void init() {
        try {
            ...
            properties.load(ReorderService.class.getClassLoader().getResourceAsStream("video_train_model6.properties"));
    ...
    private String parseRankAlbum(String json) {
        ...
        for (int i = 0; i < array.size(); i++) {
            ...
            String playDate = object.getString("playDate");
            if (playDate != null && playDate.length() > 4) {
                playDate = playDate.substring(0, 4);
            }
            tmp.put("playDate", playDate);
    ...
    private void parseIdAndFeas(JSONArray array, List<String> ids, List<String> feas) {
        ...
        for (int i = 0; i < array.size(); i++) {
            ...
            feas4 = o.getString("playDate");
            if (feas4 != null && feas4.length() > 4) {
                feas4 = feas4.substring(0, 4);
            }
            if(feas4.length()<4){
                feas4 ="1999";
            }
            String line = "1" + SPLIT + feas4;// + BLANK + "2" + SPLIT + feas2 + BLANK + "3" + SPLIT + feas3 + BLANK + "4" + SPLIT + feas4;
            feas.add(line);
        }
    }
