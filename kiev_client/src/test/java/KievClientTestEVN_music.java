 ...
 *
 */
public class KievClientTestEVN_music {
    private ApplicationContext ctx;
    private MusicSearchService musicSearchService;

    @Before
    public void setUp() throws Exception {
        ctx = new ClassPathXmlApplicationContext("remote-rpc-client-test-music.xml");
        //searchAppStub = (SearchService) ctx.getBean("searchService");
        musicSearchService = (MusicSearchService) ctx.getBean("musicSearchService");
        try {
            Thread.sleep(1000);
    ...
    public void testSearchAblums() {
        ...
        param.setKeyword("Ageha Dance");
        param.setPage(0);
        param.setLimit(10);
        MusicSearchService.Result result = musicSearchService.searchSong(param);
     ...
     *
     */
    public static class KievClientTestEVN_solr {
        private ApplicationContext ctx;
