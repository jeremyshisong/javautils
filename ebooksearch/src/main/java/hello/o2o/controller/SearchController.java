...
import com.meizu.galaxy2.biz.api.EBookSearchService;
        ...
        try {
            if ("business".equals(radio)) {
                if (StringUtils.isNotBlank(paramsVO.getKeyword())) {
                    EBookSearchService.Param param = new EBookSearchService.Param();
                    BeanCopyer.copy(paramsVO, param);
                    results = eBookSearchService.search(param);
                }
            } else {
                EBookSearchService.Param param = new EBookSearchService.Param();
                BeanCopyer.copy(paramsVO, param);
                ...
                results = eBookSearchService.suggest(param);
            }
        } catch (Exception e) {
        }
        if (results == null) {
